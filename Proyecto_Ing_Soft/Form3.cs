...
namespace Proyecto_Ing_Soft
{
    public partial class Form3 : Form
    {
        ...
        {
            if (txtIdMateria.Text != "" && txtNombreM.Text != "" && txtUnidades.Text != "" && txtIdCarreraM.Text != "" && txtHoras.Text != "" && txtCreditos.Text != "")
            {
                if (c.MateriaRegistrada(txtIdMateria.Text) == 0)
                {
                    c.insertarM(txtIdMateria.Text, txtNombreM.Text, txtHoras.Text, txtIdCarreraM.Text, txtCreditos.Text, txtUnidades.Text);
        ...
        private void btnBuscarM_Click(object sender, EventArgs e)
        {
            if (c.MateriaRegistrada(txtIdMateria.Text) > 0)
            {
                c.llenarTextBoxConsultaMateria(txtIdMateria.Text, txtNombreM.Text, txtHoras.Text, txtIdCarreraM, );
            }
            else
            {
                MessageBox.Show("La carrera no existe", "No existe", MessageBoxButtons.OK, MessageBoxIcon.Error);
